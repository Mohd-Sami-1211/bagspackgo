...
export async function POST(request) {
    try {
        ...
        await dbConnect();
        let booking = await TrekBooking.findOne({ _id: bookingId, user: user.userId });
        ...
        try {
            ...
            console.error('Could not fetch trek payment for verification:', error);
        ...
        try {
            validateCapturedTrekPayment(booking, payment);
        } catch (error) {
        ...
        if (booking.status === 'refund_initiated') {
            ...
            return NextResponse.json({ success: false, paymentCaptured: true, refundInitiated: true, bookingId }, { status: 409 });
        }
        if (['cancelled', 'cancellation_requested'].includes(booking.status)) {
            ...
            const refund = await refundTrekBookingPayment(booking, razorpay_payment_id);
            return NextResponse.json({
                success: false,
        ...
        console.error('Trek verify error:', error);
