...
export async function GET(req, context) {
    ...
    try {
        ...
        if (trip) {
            return NextResponse.json({ success: true, type: 'trip', data: { ...trip, bookingType: 'trip' } });
        }

        // Check Trek Bookings
        const trek = await TrekBooking.findOne(query).lean();
        if (trek) {
            return NextResponse.json({ success: true, type: 'trek', data: { ...trek, bookingType: 'trek' } });
        }
        ...
        if (isValidId) {
            ...
            if (eventBooking) {
                ...
                const gd = e.guide ? await GuideDetails.findOne({ guide: e.guide._id }).select('companyname').lean() : null;
                ...
                const formattedBooking = {
                    ...
                    companyName: gName,
                    guide: gName,
                    providerPhone: e.guide?.phone,
                    providerEmail: e.guide?.email,
