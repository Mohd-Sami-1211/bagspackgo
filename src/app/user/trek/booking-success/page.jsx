...
import { CheckCircle2, Home, MapPin, Calendar, Users, CreditCard, ChevronRight, Hash, User, Clock, Navigation, QrCode, Download, Eye, EyeOff, Ticket, Instagram, Facebook, Mail, Phone, Globe, Mountain } from 'lucide-react';
...
function BookingSuccessContent() {
    ...
    const handleDownloadPDF = () => {
        ...
        window.open(`/user/trek/pass/${bookingId}?print=true`, '_blank');
    };
                    ...
                    >
                        ...
                        <div className="text-center mb-8">
                            ...
                            <p className="text-emerald-700 font-medium px-4 py-1 bg-emerald-100 inline-block rounded-full text-xs">
                                Payment successful & Trek Booked
                            </p>
                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6 flex flex-col sm:flex-row items-stretch">
                            <div className="flex-1 p-6 flex flex-col justify-center border-b sm:border-b-0 sm:border-r border-gray-200 bg-white">
                                ...
                                <div className="flex flex-wrap gap-2 mt-4">
                                    ...
                                    <button 
                                         onClick={() => {
                                             if (bookingId) window.open(`/user/trek/pass/${bookingId}`, '_blank');
                                         }}
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
                            ...
                            {/* Trek Summary Details */}
                            <div className="p-6 sm:p-8 border-b border-gray-200 relative">
                                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-6 relative z-10">Trek Summary</h3>
                                ...
                                <div className="bg-slate-50 border border-gray-200 rounded-lg p-5 mb-6 relative z-10 flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between">
                                    <div>
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                                            <Mountain className="w-3.5 h-3.5"/> Trek Name
                                        </p>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 relative z-10">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-slate-400"/> Trekkers</p>
                                        <p className="font-semibold text-slate-900 text-sm leading-tight">{numPeople || 1} Pax</p>
                                    </div>
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <p className="font-semibold text-slate-900 text-sm leading-tight">{pickupDropoff?.pickup?.time || 'Pending'}</p>
                                    </div>
                                    <div className="flex flex-col gap-1.5">
                            ...
                            <div className="p-6 sm:p-8 bg-slate-50">
                                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-6 flex items-center gap-2">
                                    <User className="w-4 h-4 text-emerald-600" /> Confirmed Trekkers
                                </h3>
                                <div className="grid sm:grid-cols-2 gap-4">
                                    {travelers.length > 0 ? travelers.map((t, idx) => (
                                        <div key={idx} className="flex flex-col p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                                            <p className="font-bold text-slate-900 text-sm mb-1">{t.name || "Unnamed Trekker"}</p>
                                        ...
                                        <p className="text-sm font-medium text-slate-500 italic col-span-2">No specific trekker names recorded.</p>
                                    )}
                                </div>
                            </div>
