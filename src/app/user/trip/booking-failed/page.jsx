...
const COPY = {
  failed: {
    ...
    description: "No confirmed payment was found for this attempt. Your trip details are still saved.",
    tone: "bg-red-500",
  },
  processing: {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (result.state === "confirmed") {
        ...
        router.replace(`/user/trip/booking-success?bookingId=${bookingId}&ref=${encodeURIComponent(result.bookingRef || "")}`);
        return;
      }
          ...
          <div className="space-y-3">
            ...
            {state === "failed" && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-black text-white font-bold py-4 rounded-2xl"
              >
            ...
            >
              <ArrowLeft className="w-5 h-5 text-gray-400" /> Back to trips
            </button>
          </div>
        </div>
      </motion.div>
