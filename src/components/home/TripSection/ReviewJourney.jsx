...
import { 
  ArrowLeft, CheckCircle2, AlertCircle, ChevronDown, ChevronUp,
  MapPin, Calendar, Clock, Hotel, Users, Luggage, Navigation,
  ShieldCheck, Tag, CreditCard, ChevronRight, User, Mail, Phone
} from 'lucide-react';
...
const ReviewJourney = ({ guide, searchParams, tripData: propTripData }) => {
  ...
  const calculatePayment = () => {
    ...
    if (selectedPkg?.pricingTiers && selectedPkg.pricingTiers.length > 0) {
      ...
      perPersonPrice = Number(selectedPkg?.price?.individual || selectedPkg?.price || guide?.price || 0);
    }
  ...
  const handleMakePayment = async () => {
    ...
    if (!packageId || !guideId) {
      setPaymentError('Missing required data. Please go back and select a package again.');
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-sm font-semibold text-gray-400 uppercase tracking-widest">Preparing Summary...</p>
      </div>
    );
  }
  ...
  if (!tripData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center py-20 text-center px-4">
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-2">No Booking Data Found</h2>
        <p className="text-sm text-gray-500 mb-6">It seems your booking session expired. Please start again.</p>
        <button onClick={() => router.back()} className="px-6 py-2.5 bg-emerald-600 text-white rounded-full font-bold shadow-md shadow-emerald-200 hover:bg-emerald-700 transition">
          Go Back
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#F0FDF4]/30 pt-0 sm:pt-2 pb-24 lg:pb-8 px-4 sm:px-6 lg:px-8 font-sans selection:bg-emerald-200 relative -mt-16 sm:-mt-10 md:-mt-12 lg:-mt-14">
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 relative z-10">
        ...
        <div className="flex items-center gap-4 mb-4 sm:mb-8">
          ...
          <div>
            <p className="text-[10px] sm:text-xs font-bold text-emerald-600 uppercase tracking-[0.15em] mb-0.5">Checkout</p>
            <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight leading-none">Review Booking</h1>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-emerald-50/50 p-6 sm:p-8 overflow-hidden relative">
          ...
          <div className="mb-6 pb-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
             <div>
               <h2 className="text-2xl font-black text-gray-900 leading-tight mb-1">
                 {tripData?.selectedPackage?.label || tripData?.selectedPackage?.name || "Trip Package"}
               </h2>
             ...
             <div className="bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100/50 w-fit shrink-0">
                <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-widest mb-0.5">Travel Dates</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
             <div className="flex flex-col">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-gray-400" /> Duration</p>
                <p className="font-bold text-gray-800 text-sm sm:text-base">{days} Days</p>
             </div>
             <div className="flex flex-col">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Hotel className="w-3.5 h-3.5 text-gray-400" /> Stay</p>
                <p className="font-bold text-gray-800 text-sm sm:text-base">{Math.max(1, days - 1)} Nights</p>
             </div>
             <div className="flex flex-col">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-gray-400" /> Guests</p>
                <p className="font-bold text-gray-800 text-sm sm:text-base">{count} Pax</p>
             </div>
             <div className="flex flex-col">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Luggage className="w-3.5 h-3.5 text-gray-400" /> Category</p>
                <p className="font-bold text-gray-800 text-sm sm:text-base capitalize">{category}</p>
             </div>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 sm:p-8 flex flex-col">
             ...
             <div className="flex-1 flex flex-col relative gap-8 ml-2">
                ...
                <div className="relative z-10 pl-8">
                   ...
                   <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-1 line-clamp-1 truncate pr-2">
                      Pickup • {arrivalDeparture?.pickup?.time || "TBD"}
                   </p>
                   <p className="font-bold text-gray-900 text-sm line-clamp-1" title={arrivalDeparture?.pickup?.address?.label || arrivalDeparture?.pickup?.address || "No location chosen"}>
                     {arrivalDeparture?.pickup?.address?.label || arrivalDeparture?.pickup?.address || "No location chosen"}
                   </p>
                   {arrivalDeparture?.pickup?.location && (
                      <p className="text-xs font-semibold text-gray-500 mt-0.5">City: {arrivalDeparture.pickup.location?.label || arrivalDeparture.pickup.location}</p>
                   )}
                </div>
                ...
                <div className="relative z-10 pl-8">
                   ...
                   <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1 line-clamp-1 truncate pr-2">
                      Drop Off • {arrivalDeparture?.dropoff?.time || "TBD"}
                   </p>
                   <p className="font-bold text-gray-900 text-sm line-clamp-1" title={arrivalDeparture?.dropoff?.address?.label || arrivalDeparture?.dropoff?.address || "No location chosen"}>
                     {arrivalDeparture?.dropoff?.address?.label || arrivalDeparture?.dropoff?.address || "No location chosen"}
                   </p>
                   {arrivalDeparture?.dropoff?.location && (
                      <p className="text-xs font-semibold text-gray-500 mt-0.5">City: {arrivalDeparture.dropoff.location?.label || arrivalDeparture.dropoff.location}</p>
                   )}
                </div>
             </div>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden flex flex-col">
             <div className="px-6 py-5 sm:px-8 border-b border-gray-50 flex items-center justify-between">
               ...
               <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md text-xs font-bold">{travelers.length} Pax</span>
             </div>
                   ...
                   <div key={idx} className="flex justify-between items-center group">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 rounded-full bg-emerald-50 text-emerald-700 font-bold text-xs flex items-center justify-center border border-emerald-100 shrink-0">
                           {t.name ? t.name.charAt(0) : <User className="w-3.5 h-3.5"/>}
                         </div>
                         <div>
                            <p className="text-sm font-bold text-gray-900 line-clamp-1 max-w-[120px]">{t.name || "Unnamed"}</p>
                            <p className="text-[11px] font-semibold text-gray-500 flex items-center gap-1.5 capitalize">
                               {t.gender?.label || t.gender || "-"} <span className="w-1 h-1 rounded-full bg-gray-300"></span> {t.age ? `${t.age} yrs` : "-"}
                            </p>
                         </div>
                      </div>
                      {t.idType && (
                         <div className="text-right">
                           ...
                           <p className="text-xs font-mono font-bold text-gray-700 mt-0.5">{t.idNumber}</p>
                         </div>
                      )}
                   </div>
                )) : (
                   <div className="h-full flex flex-col items-center justify-center py-4">
                     <p className="text-xs font-medium text-gray-400">No traveler data</p>
                   </div>
                )}
             </div>
             ...
             <div className="bg-gray-50 w-full px-6 py-4 flex flex-wrap gap-x-6 gap-y-2 text-[11px] font-semibold text-gray-500 tracking-wide border-t border-gray-100">
               <span className="flex items-center gap-1.5"><Mail className="w-3 h-3.5" /> {personalDetails?.contactDetails?.email || "No Email"}</span>
               <span className="flex items-center gap-1.5"><Phone className="w-3 h-3.5" /> +91 {personalDetails?.contactDetails?.mobile || "No Number"}</span>
             </div>
          </div>
               ...
               <div className="p-6 sm:p-8 border-b md:border-b-0 md:border-r border-gray-100 bg-gray-50/50 flex flex-col justify-between">
                  <div>
                    <h3 className="text-sm font-black text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
                       <Tag className="w-4 h-4 text-emerald-500" /> Have a Promo Code?
                    </h3>
                    <div className="flex flex-col gap-3">
                      {!appliedCoupon ? (
                         <div className="flex gap-2 relative">
                           ...
                           />
                           <button onClick={handleApplyCoupon} className="absolute right-1.5 top-1.5 bottom-1.5 bg-gray-900 hover:bg-gray-800 text-white font-bold rounded-xl px-5 transition text-xs shadow-sm active:scale-95">
                             Apply
                           </button>
                         </div>
                      ) : (
                         <div className="flex items-center justify-between bg-emerald-50/80 border border-emerald-200/60 px-5 py-3.5 rounded-2xl shadow-sm transition-all">
                            <div className="flex items-center gap-2">
                               <div className="bg-emerald-500 p-1 rounded-full"><CheckCircle2 className="w-3 h-3 text-white" /></div>
                               <span className="font-bold text-emerald-800 text-sm tracking-tight">{appliedCoupon.code}</span>
                            </div>
                            <button onClick={removeCoupon} className="text-xs font-bold text-emerald-600 hover:text-emerald-800 underline uppercase tracking-wider bg-transparent">Remove</button>
                         </div>
                      )}
                      {couponMessage && (
                        <p className={`text-[11px] font-bold mt-1 px-1 ${appliedCoupon ? 'text-emerald-600' : 'text-red-500'}`}>{couponMessage}</p>
                      )}
                    </div>
                  </div>
               ...
               <div className="p-6 sm:p-8 flex flex-col">
                  ...
                  <div className="space-y-4 flex-1">
                     <div className="flex justify-between items-center text-sm font-semibold text-gray-600">
                        ...
                        <span className="text-gray-900">₹{paymentDetails.packageAmount.toLocaleString('en-IN')}</span>
                     </div>
                     ...
                     {paymentDetails.discount > 0 && (
                        <div className="flex justify-between items-center text-sm font-bold text-emerald-600 bg-emerald-50/50 p-2.5 rounded-xl -mx-2.5">
                          ...
                          <span>-₹{paymentDetails.discount.toLocaleString('en-IN')}</span>
                        </div>
                     )}
                     ...
                     <div className="border border-gray-100 rounded-xl overflow-hidden bg-gray-50/30">
                       ...
                       >
                         <div className="flex items-center gap-1.5">
                           Service & Gateway Fees <span className="text-[10px] bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded font-bold uppercase">(Incl GST)</span>
                         </div>
                         <div className="flex items-center gap-2">
                           <span className="text-gray-900">₹{paymentDetails.convenienceFees.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                           {showFeeBreakdown ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                         </div>
                       </button>
                           ...
                           >
                              <div className="p-4 flex flex-col gap-3 text-xs font-medium text-gray-500">
                                 <div className="flex justify-between">
                                    ...
                                    <span>₹{paymentDetails.platformFee.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                                 </div>
                                 <div className="flex justify-between">
                                    <span>Payment Gateway Charge (2%)</span>
                                    <span>₹{paymentDetails.gatewayCharges.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                                 </div>
                                 <div className="flex justify-between">
                                    <span>GST on Gateway (18%)</span>
                                    <span>₹{paymentDetails.gstOnGateway.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                                 </div>
                              </div>
                           </motion.div>
                         )}
                  ...
                  <div className="pt-6 mt-2 border-t border-gray-100">
                     <div className="flex justify-between items-end mb-6">
                        <div className="flex flex-col">
                           ...
                           <span className="font-black text-gray-900 text-3xl leading-none tracking-tight">₹{Math.round(paymentDetails.totalAmount).toLocaleString('en-IN')}</span>
                        </div>
                     </div>
                     ...
                     <button 
                       ...
                       className="w-full h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-lg transition-all flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-75 disabled:cursor-not-allowed shadow-[0_10px_20px_rgba(16,185,129,0.25)] group"
                     >
                       {isPaymentLoading ? (
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] lg:hidden z-50 flex flex-col origin-bottom rounded-t-3xl transition-transform">
               ...
               >
                 <div className="p-5 flex flex-col gap-4">
                     ...
                     <div className="flex flex-col gap-2 pt-3 border-t border-gray-200">
                       <p className="text-xs font-bold text-gray-800 uppercase tracking-widest">Service & Gateway Fees (Incl GST)</p>
                       <div className="flex justify-between items-center text-xs font-semibold text-gray-500 pl-2">
                          <span>Platform Fee (1%)</span>
                          <span className="text-gray-900">₹{paymentDetails.platformFee.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                       </div>
                       <div className="flex justify-between items-center text-xs font-semibold text-gray-500 pl-2">
                          <span>Payment Gateway Charge (2%)</span>
                          <span className="text-gray-900">₹{paymentDetails.gatewayCharges.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                       </div>
                       <div className="flex justify-between items-center text-xs font-semibold text-gray-500 pl-2">
                          <span>GST on Gateway (18%)</span>
                          <span className="text-gray-900">₹{paymentDetails.gstOnGateway.toLocaleString('en-IN', {maximumFractionDigits: 1})}</span>
                       </div>
                     </div>

                     {/* Coupon Input Mobile */}
                     <div className="mt-4 flex flex-col gap-3">
                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Have a Promo Code?</label>
                        {!appliedCoupon ? (
                           <div className="flex gap-2">
                              ...
                              />
                              <button onClick={handleApplyCoupon} className="bg-gray-900 hover:bg-gray-800 text-white font-bold rounded-xl px-4 text-xs shadow-sm active:scale-95">
                                Apply
                              </button>
                           </div>
                        ) : (
                           <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 px-3 py-2.5 rounded-xl shadow-sm">
                              <div className="flex items-center gap-1.5">
                                 <div className="bg-emerald-500 p-0.5 rounded-full"><CheckCircle2 className="w-3 h-3 text-white" /></div>
                                 <span className="font-bold text-emerald-800 text-xs tracking-tight">{appliedCoupon.code}</span>
                              </div>
                              <button onClick={removeCoupon} className="text-[10px] font-bold text-emerald-600 underline uppercase tracking-wider">Remove</button>
                           </div>
                        )}
                        {couponMessage && <p className={`text-[10px] font-bold mt-0.5 px-0.5 ${appliedCoupon ? 'text-emerald-600' : 'text-red-500'}`}>{couponMessage}</p>}
                     </div>
                 </div>
               </motion.div>
            )}
         ...
         <div className="p-4 sm:p-5 flex items-center justify-between gap-4 bg-white">
            <div className="flex items-center gap-3">
               ...
               >
                  {showFeeBreakdown ? <ChevronDown className="w-5 h-5" /> : <ChevronUp className="w-5 h-5" />}
               </button>
               <div className="flex flex-col">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Total Payable</p>
                  <p className="font-black text-gray-900 text-xl tracking-tight leading-tight mt-0.5">₹{Math.round(paymentDetails?.totalAmount || 0).toLocaleString('en-IN')}</p>
               </div>
            </div>
            <button 
               onClick={handleMakePayment}
               disabled={isPaymentLoading}
               className="flex-1 h-12 bg-emerald-600 text-white rounded-xl font-black tracking-wide shadow-[0_5px_15px_rgba(16,185,129,0.3)] flex items-center justify-center gap-2 active:scale-[0.98] transition disabled:opacity-75"
            >
               {isPaymentLoading ? (
