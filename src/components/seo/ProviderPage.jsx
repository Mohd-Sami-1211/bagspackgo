...
export function providerMetadata(data) {
  ...
  return pageMetadata({ title: p.name + ' — Trips, Treks & Events', description: p.bio || 'Explore trips, treks and events hosted by ' + p.name + ' on Bagspackgo.', path: providerPath(p), image: p.logo });
}
export default function ProviderPage({ data }) {
  ...
  return <>
    ...
    <JsonLd data={breadcrumbs([{ name: 'Local travel companies', path: '/providers' }, { name: p.name, path }])} />
