...
/* ─── filter tabs ─────────────────────────────────────── */
const TABS = [
  { key: 'confirmed', label: 'Upcoming',  Icon: Luggage      },
...
function BookingCard({ booking, index }) {
  ...
  return (
    ...
    >
      <Link href={`/serviceprovider/dashboard/trips/${booking.id}`}>
...
export default function TripMainContent() {
  ...
  const load = async () => {
    ...
    try {
      const r = await fetch('/api/provider/trip-bookings');
    ...
    <div className="w-full space-y-5 pb-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center shadow shadow-emerald-200 shrink-0">
            <Luggage className="w-3.5 h-3.5 text-white" />
          </div>
          <div className="min-w-0">
            <h1 className="text-[17px] font-black text-gray-900 tracking-tight truncate">Trip Bookings</h1>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Link
            href="/serviceprovider/dashboard/settings/packages?tab=trip"
      ...
      {/* Mobile search (shown only on small screens, below header) */}
      ...
      {/* ── Filter tabs (full width, no swipe needed) ─────── */}
      ...
      {loading ? (
        ...
        >
          <div className="w-14 h-14 rounded-2xl bg-white shadow-md border border-gray-100 flex items-center justify-center">
            <Luggage className="w-6 h-6 text-gray-300" />
          </div>
