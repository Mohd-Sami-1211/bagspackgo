...
export default function OffBeatDetailsPage({ id, initialData }) {
    ...
    const { data, isLoading, error } = useOffbeatDetail(id, { fallbackData: { success: true, data: initialData }, revalidateOnMount: false });
                ...
                {galleryOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[220] overflow-y-auto bg-[#07110e]/96 px-4 py-6 text-white backdrop-blur-xl sm:px-6 sm:py-8" role="dialog" aria-modal="true" aria-label="Full destination gallery">
                        <div className="mx-auto max-w-7xl">
                            <div className="mb-7 flex items-center justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-[#e1c48e]">{offbeat.destination}</p><h2 className="mt-2 font-serif text-3xl tracking-[-.03em] sm:text-4xl text-[#e1c48e]">Full gallery</h2></div><button type="button" onClick={() => setGalleryOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-black shadow-lg transition hover:bg-gray-200" aria-label="Close gallery"><X className="h-5 w-5" /></button></div>
