...
const getPublicProviders = unstable_cache(async () => {
  await dbConnect();
  const [accounts, details] = await Promise.all([
    Guide.find({ isActive: { $ne: false } }).select('_id username applicationStatus isActive updatedAt').lean(),
    GuideDetails.find({}).select('guide companyname profileSlug bio destinationId speciality status pausedServices rating reviews languages totalTrips updatedAt').lean(),
  ]);
...
export const getPublicCatalog = unstable_cache(async () => {
  ...
  const [packages, offbeats, events] = await Promise.all([
    Package.find({ status: 'active', category: 'trip', provider: { $in: tripIds } })
      .select('name destination category days aboutPackage updatedAt provider').sort({ updatedAt: -1 }).lean(),
...
export const getPublicPackage = unstable_cache(async (id) => {
  ...
  await dbConnect();
  const pkg = await Package.findOne({ _id: id, status: 'active', category: 'trip' })
    .select('-packagePhotos -itinerary.hotelPhotos -itinerary.destinationPhotos').lean();
  if (!pkg) return null;
  const providers = await getPublicProviders();
  ...
  return serialize({ ...pkg, provider: {
    ...
    bio: provider.bio || '', rating: provider.rating, reviews: provider.reviews, languages: provider.languages, totalTrips: provider.totalTrips, logo: '/api/public/provider/' + provider.guide + '/logo',
  } });
